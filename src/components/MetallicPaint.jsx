...
export function parseLogoImage(file) {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = function () {
      ...
      const C = 0.01;
      const ITERATIONS = 150; // Reduced from 300 for faster processing
...
vec2 get_img_uv() {
    vec2 img_uv = vUv;
    img_uv -= .5;
  // allow scaling the image UVs so the logo can appear smaller inside the canvas
  img_uv *= u_img_scale;
...
void main() {
    ...
    color = vec3(r, g, b);
    color *= opacity;
  // u_show controls whether the image is revealed (0 = hidden, 1 = visible)
  fragColor = vec4(color * u_show, opacity * u_show);
}
`;

export default function MetallicPaint({ imageData, params = defaultParams }) {
  const canvasRef = useRef(null);
  const glRef = useRef(null); // Use ref instead of state to avoid async race condition
  const uniformsRef = useRef({});
  // NOTE: using refs for gl/uniforms to avoid race conditions
